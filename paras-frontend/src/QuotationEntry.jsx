...
import { getParts, getAccounts, createQuotation } from './api';
import { FaFileContract, FaShoppingCart } from 'react-icons/fa';
import AccountView from './AccountView';

export default function QuotationEntry({ quotation, onBack, onClose }) {

  const [formData, setFormData] = useState(quotation ? {
    partyCd: quotation.partyCd || '',
  ...
  const [filter, setFilter] = useState({
    brand: '', partItem: '', model: '', description: ''
  });
  ...
  const addItem = (part) => {
    const rate = parseFloat(part.wholesaleFinal || part.wholesalePrice || 0);
  ...
  return (
    <div style={{ width: '100%', height: '100vh', display: 'flex', flexDirection: 'column', background: '#dfe8ef', fontFamily: 'Tahoma,Verdana,sans-serif', fontSize: '11px', color: '#1d2d5a', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '0 20px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', minWidth: '200px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', justifyContent: 'flex-end' }}>
              ...
              <input type="date" name="quotationDate" value={formData.quotationDate} onChange={handleChange} style={{ ...inputStyle, flex: 'none', width: '130px' }} />
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ background: '#d4edf9', borderBottom: '1px solid #7a9cbf', padding: '4px 8px', display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginRight: '10px' }}>
            <div style={{ background: '#003399', padding: '3px 5px', borderRadius: '2px' }}>
              <FaFileContract size={14} color="white" />
            </div>
            <strong style={{ color: '#003399', fontSize: '12px' }}>QUOTATION</strong>
          </div>
      ...
      {/* MODALS (Simplified versions of SalesOrderEntry modals) */}
      {showModal && (
        <div style={overlayStyle}>
          <div style={{ ...modalStyle, width: '800px' }}>
            ...
            <div style={{ padding: '8px 10px', display: 'flex', gap: '8px', background: '#eef3f7' }}>
              <input placeholder="Search part..." value={filter.partItem} onChange={e => setFilter(p => ({ ...p, [filter.partItem]: e.target.value }))} style={inputStyle} />
            </div>
            <div style={{ height: '300px', overflow: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead style={{ background: '#7fc6ea' }}>
                  <tr><th>Brand</th><th>Part No</th><th>Description</th><th>Price</th></tr>
                </thead>
                <tbody>
                  {filteredParts.map((p, idx) => (
                    <tr key={idx} onClick={() => addItem(p)} style={{ cursor: 'pointer', background: idx % 2 === 0 ? '#fff' : '#f0f8ff' }}>
                      ...
                      <td style={tdS()}>{p.wholesalePrice}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
